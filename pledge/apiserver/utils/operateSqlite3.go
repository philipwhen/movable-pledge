...
func (c *appContext) InsertTable(info define.PledgeWarningInfo) error {
	...
	stmt, err := c.db.Prepare("insert into PledgeWarningInfo(PledgeNoStorage, ChannelSeq, PledgeName, SocialCreditCode, CoreEnterpriseName, WarningMsg) values(?,?,?,?,?,?)")
	if err != nil {
		Log.Errorf("Sqlite3 insert table prepare error , %s", err.Error())
		return err
	}
	defer stmt.Close()
	_, err = stmt.Exec(info.PledgeNoStorage, info.ChannelSeq, info.PledgeName, info.SocialCreditCode, info.CoreEnterpriseName, info.WarningMsg)
	if err != nil {
...
func (c *appContext) QueryTable(pledgeNoStorage string) (bool, define.PledgeWarningInfo, error) {
	...
	err := rows.Scan(&info.PledgeNoStorage, &info.ChannelSeq, &info.PledgeName, &info.SocialCreditCode, &info.CoreEnterpriseName, &info.WarningMsg)
	if err != nil {
