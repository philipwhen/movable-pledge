...
func (c *appContext) QueryListCnt(blackListCntType string) (uint64, bool, error) {
	var listCnt uint64
	exist := false
	listCnt = 0

	rows, err := c.db.Query("select ListCnt from BlackListCnt where Type = ?", blackListCntType)
	if err != nil {
		logger.Errorf("Sqlite3 query table error , %s", err.Error())
		return 0, exist, err
	}
