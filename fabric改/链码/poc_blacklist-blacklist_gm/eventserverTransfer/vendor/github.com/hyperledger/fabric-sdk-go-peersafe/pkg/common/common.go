...
func ProcessProposal(peerClients []*cp.PeerClient, signedProp *protos_peer.SignedProposal) (proposalResps []*protos_peer.ProposalResponse, err error) {
	for _, client := range peerClients {
		proposalResp, err := client.ProcessProposal(signedProp, true)
		if err != nil {
			return proposalResps, err
		}
		proposalResps = append(proposalResps, proposalResp)
	}
	if proposalResps == nil {
...
func Broadcast(ordererClients []*orderer.OrdererClient, proposal *protos_peer.Proposal, resps []*protos_peer.ProposalResponse) error {
	...
	for _, client := range ordererClients {
		err = client.BroadcastClientSend(env, true)
		if err != nil {
			return err
		}
	}
