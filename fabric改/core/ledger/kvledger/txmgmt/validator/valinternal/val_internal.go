...
func (u *PubAndHashUpdates) ApplyWriteSet(txRWSet *rwsetutil.TxRwSet, txHeight *version.Height, v *Validator) {
	for _, nsRWSet := range txRWSet.NsRwSets {
		...
		for _, kvWrite := range nsRWSet.KvRwSet.Writes {
			if kvWrite.IsDelete {
				...
				if kvWrite.Value[0] == 1 {
					vv := u.PubUpdates.Get(ns, kvWrite.Key)
					if vv == nil {
						versionedValue, _ := v.db.GetState(ns, kvWrite.Key)
						if versionedValue == nil {
							u.PubUpdates.Put(ns, kvWrite.Key, kvWrite.Value, txHeight)
						} else {
							tmp1 := bytesToInt64(versionedValue.Value[1:])
							tmp2 := bytesToInt64(kvWrite.Value[1:])
							tmp3 := tmp2 + tmp1
							rv := int64ToBytes(tmp3)
							rv[0] = 1
							u.PubUpdates.Put(ns, kvWrite.Key, rv, txHeight)
						}
					} else {
						tmp1 := bytesToInt64(vv.Value[1:])
						tmp2 := bytesToInt64(kvWrite.Value[1:])
						tmp3 := tmp2 + tmp1
						rv := int64ToBytes(tmp3)
						rv[0] = 1
