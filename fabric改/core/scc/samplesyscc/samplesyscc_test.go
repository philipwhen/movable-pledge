...
func (s *mockStub) GetState(key string) ([]byte, error, int) {
	args := s.Called(key)
	if args.Get(1) == nil {
		return args.Get(0).([]byte), nil, 3
	}
	return nil, args.Get(1).(error), 3
}

func (s *mockStub) PutState(key string, value []byte, it int) error {
	args := s.Called(key)
	if args.Get(0) == nil {
		return nil
	}
...
func (s *mockStub) SetSysType(iS bool) {
	panic("implement me")
}
