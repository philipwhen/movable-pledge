...
func (stub *ChaincodeStub) SetSysType(iS bool) {
	stub.IsSys = iS
}
...
func (stub *ChaincodeStub) GetState(key string) ([]byte, error, byte) {
	value, err := stub.handler.handleGetState(key, stub.TxID)
	if stub.IsSys {
		return value, err, 3
	}
	if err != nil {
	...
	return value[1:], err, value[0]
}

// PutState documentation can be found in interfaces.go
func (stub *ChaincodeStub) PutState(key string, value []byte, it int) error {
	if key == "" {
