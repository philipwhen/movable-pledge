...
func (t *shimTestCC) Init(stub ChaincodeStubInterface) pb.Response {
	...
	err = stub.PutState(A, []byte(strconv.Itoa(Aval)), true)
	if err != nil {
		return Error(err.Error())
	}

	err = stub.PutState(B, []byte(strconv.Itoa(Bval)), true)
	if err != nil {
		return Error(err.Error())
	}
...
func (t *shimTestCC) invoke(stub ChaincodeStubInterface, args []string) pb.Response {
	...
	Avalbytes, err, _ := stub.GetState(A)
	if err != nil {
	...
	Bvalbytes, err, _ := stub.GetState(B)
	if err != nil {
		return Error("Failed to get state")
	}
	if Bvalbytes == nil {
