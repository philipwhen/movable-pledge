...
func (stub *MockStub) SetSysType(iS bool) {
	iS = iS
}
...
func (stub *MockStub) GetState(key string) ([]byte, error, byte) {
	value := stub.State[key]
	if stub.Name == "lscc" || stub.Name == "validatoronevalidsignature" {
		return value, nil, 3
	}
	if len(value) == 0 {
		return nil, nil, 0
...
func (iter *MockStateRangeQueryIterator) Next() (*queryresult.KV, error) {
	...
	for iter.Current != nil {
		...
		if (comp1 >= 0 && comp2 <= 0) || (iter.StartKey == "" && iter.EndKey == "") {
			...
			value, err, _ := iter.Stub.GetState(key)
